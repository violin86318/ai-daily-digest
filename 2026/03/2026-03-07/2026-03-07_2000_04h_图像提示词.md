```plaintext
Create a high-density, professional information design infographic for Xiaohongshu about「2026年3月 AI禁令悖论：禁用与使用的矛盾」.
...
图片1 → 核心主题：AI禁令悖论：禁用与使用
├─ 坐标 A-01：Trump 禁令（立即停用 vs 6个月过渡）
├─ 坐标 B-05：伊朗行动仍在使用
├─ 坐标 C-12：供应链风险标签
├─ 坐标 D-03：自主战争分歧
├─ 坐标 E-08：诉讼倒计时
├─ 坐标 F-11：Dario 效应（道德 icon）
└─ 坐标 G-02：编者观察（政策矛盾）
