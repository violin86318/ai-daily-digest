...
```
图片1 → 核心主题：英伟达GTC 2026算力革命
├─ 坐标 A-01：算力市场（1万亿美元）
├─ 坐标 B-05：Vera Rubin平台（144 GPU）
├─ 坐标 C-12：NemoClaw（Agent操作系统）
├─ 坐标 D-03：Project GR00T（人形机器人）
├─ 坐标 E-08：CUDA 20周年（数亿装机）
└─ 坐标 F-11：马斯克点赞中国AI
```
...
```
Create a high-density, professional information design infographic for Xiaohongshu about「英伟达GTC 2026：算力革命与物理AI大爆炸」.
